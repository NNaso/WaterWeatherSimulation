...
{
    public sealed class WaterWeatherSimulationModSystem : ModSystem
    {
        ...
        private void EventOnBeginChunkColumnLoadChunkThread(IServerMapChunk mapChunk, int chunkX, int chunkZ, IWorldChunk[] chunks)
        {
            ...
            var chunkSize = 32;
            for (var x = chunkX * chunkSize; x < chunkX * chunkSize + chunkSize; x += 1)
                for (var z = chunkZ * chunkSize; z < chunkZ * chunkSize + chunkSize; z += 1)
                {
                    blockPos.X = x;
                    blockPos.Z = z;
